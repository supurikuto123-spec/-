...
class MailStore {
  ...
  async init() {
    ...
    // マイグレーション：既存のmailsテーブルにsaved, saved_at, expires_atカラムを追加
    try {
      ...
      if (!hasSaved || !hasSavedAt || !hasExpiresAt) {
        console.log('[MIGRATION] Migrating mails table schema for save functionality...');
        const now = Date.now();
        ...
        this.db.exec(`
          ...
          CREATE TABLE mails_new (
            ...
            saved_at INTEGER,
            expires_at INTEGER NOT NULL DEFAULT ${defaultExpiresAt},
            FOREIGN KEY(address) REFERENCES users(address) ON DELETE CASCADE
          );
          ...
          PRAGMA foreign_keys = ON;
        `);
        console.log('[MIGRATION] Mails table migration completed');
      }
    } catch (err) {
      console.warn('Mails migration warning:', err.message);
    }
  ...
  async saveMail(address, password, mailId) {
    ...
    const now = Date.now();
    // 保存時は expires_at を NULL に設定（削除されない）
    
    const stmt = this.db.prepare(
      'UPDATE mails SET saved = 1, saved_at = ?, expires_at = NULL WHERE address = ? AND id = ?'
    );
    const result = stmt.run(now, normalized, mailId);
    
    if (result.changes > 0) {
      this.createBackup('save-mail');
      return { success: true, savedAt: now, expiresAt: null, permanent: true };
    }
    return false;
  }
  ...
  async cleanupExpiredMails() {
    ...
    // saved=1 のメールは削除されない（expires_at が NULL になるため）
    const stmt = this.db.prepare(
  ...
  extractAuthCode(body, html) {
    ...
    // 一般的なOTPパターン（優先度順）
    const patterns = [
      // "認証コード: 123456" / "認証コードは 123456" / "認証コード 123456"
      /認証コード[\s:：は\s]*([0-9]{4,8})/i,
      // "verification code: 123456" / "verification code is 123456"
      /verification\s*code[\s:：is]*([0-9]{4,8})/i,
      // "ワンタイムパスワード: 123456" / "OTP: 123456"
      /(?:ワンタイムパスワード|OTP|one.time.password)[\s:：]+([0-9]{4,8})/i,
      // "コード: 123456" / "code: 123456"
      /(?:code|コード)[\s:：]+([0-9]{4,8})/i,
      // "123456 is your verification code" / "your code is 123456"
      /(?:is\s+your|あなたの|your\s+code\s*is)[\s:：]*\b([0-9]{4,8})\b/i,
      // "security code" / "secure code" パターン
      /(?:security|secure)[\s\w]*code[\s:：]+([0-9]{4,8})/i,
      // "2FA code" / "二段階認証" パターン
      /(?:2FA|二段階|two.factor|two.step)[\s\w]*[:：]?\s*([0-9]{4,8})/i,
      // 独立した6桁数字（周囲に空白がある）- 最後のフォールバック
      /\b([0-9]{6})\b/
    ];

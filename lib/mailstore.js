...
class MailStore {
  ...
  async cleanupExpiredMails() {
    ...
    // saved=1 のメールは対象外（保存時に2100年の期限が設定される）
    const stmt = this.db.prepare(
      'DELETE FROM mails WHERE saved = 0 AND expires_at IS NOT NULL AND expires_at < ?'
    );
  ...
  /**
   * 認証コード（OTP）を抽出
   * パターンによる判定（桁数固定ではなく、コンテキストで判断）
   * 6-10桁の数字、英数字混合コード、GitHubなどの8桁コードも対応
   */
  extractAuthCode(body, html) {
    ...
    // 認証コードパターン（コンテキスト優先、桁数は柔軟）
    const patterns = [
      // === 日本語パターン ===
      // "認証コードは 785440 です" / "Xの認証コードは785440です"
      /認証コード[\s:：は\s]+([0-9a-zA-Z]{4,12})\b/i,
      // "コード：12345678" / "コードは 123456"
      /(?:コード|code)[\s:：は\s]+([0-9a-zA-Z]{4,12})\b/i,
      // "パスコード: 1234" / "PINコード: 1234"
      /(?:パスコード|PIN|暗証番号)[\s:：は\s]+([0-9a-zA-Z]{3,12})\b/i,
      // "ワンタイムパスワードは 123456"
      /(?:ワンタイムパスワード|OTP)[\s:：は\s]+([0-9a-zA-Z]{4,12})\b/i,
      // "確認コード"
      /確認コード[\s:：は\s]+([0-9a-zA-Z]{4,12})\b/i,

      // === 英語パターン ===
      // "verification code: 785440" / "Your verification code is: 785440"
      /verification\s*code[\s:：is]*\s+([0-9a-zA-Z]{4,12})\b/i,
      // "Your code is: GH78ABC1" / "code is 785440"
      /(?:your\s+code[\s:：is]*|code\s*is[\s:：]*)\s+([0-9a-zA-Z]{4,12})\b/i,
      // "is your verification code" / "is your code"
      /([0-9a-zA-Z]{4,12})\s+is\s+your\s+(?:verification\s+)?code\b/i,
      // "security code" / "secure code" / "device code"
      /(?:security|secure|device)[\s\w]*code[\s:：]+([0-9a-zA-Z]{4,12})\b/i,
      // "2FA code" / "two-factor code" / "two step code"
      /(?:2FA|two.factor|two.step|two\s*step)[\s\w]*[:：]?\s*([0-9a-zA-Z]{4,12})\b/i,
      // "GitHub" / "gpg" / "recovery" code
      /(?:github|gpg|recovery|backup|access)[\s\w]*code[\s:：]+([0-9a-zA-Z]{4,12})\b/i,
      // "confirmation code"
      /confirmation\s*code[\s:：]+([0-9a-zA-Z]{4,12})\b/i,
      // "authentication code"
      /authentication\s*code[\s:：]+([0-9a-zA-Z]{4,12})\b/i,
      // "passcode" / "PIN"
      /(?:passcode|PIN)[\s:：]+([0-9a-zA-Z]{3,12})\b/i,
      // "temporary code"
      /temporary\s*code[\s:：]+([0-9a-zA-Z]{4,12})\b/i,

      // === 特殊フォーマット ===
      // "785440-" / "12345678." のような区切り付き
      /([0-9]{4,12})[-.]\s*(?:is\s+)?(?:your\s+)?(?:code|コード)/i,
      // 括弧や記号で囲まれたコード "(123456)" / "[12345678]"
      /[\(\[\{]([0-9a-zA-Z]{4,12})[\)\]\}][\s:：]*(?:code|コード)?/i,
      // "こちらのコード: AB12CD34" / "this code: 12345678"
      /(?:こちら|this)[\s\w]*[\s:：]+([0-9a-zA-Z]{6,12})\b/i
    ];
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1]) {
        ...
        // 4-12文字の英数字で、過度に単純なパターン（111111など）は除外
        if (/^[0-9A-Z]{4,12}$/.test(code) && !/^(.)(\1)+$/.test(code)) {
          return code;
        }
      }
    }

    // フォールバック：明確にOTPらしい独立した6-10桁数字
    const fallbackMatch = cleanText.match(/\b([0-9]{6,10})\b/);
    if (fallbackMatch && !/^(.)(\1)+$/.test(fallbackMatch[1])) {

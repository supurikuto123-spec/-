...
class MailStore {
  ...
  /**
   * 統計情報を取得
   * totalMailsAllTime: 削除済みも含む累計受信メール数
   */
  async getStats() {
    const userCount = this.db.prepare('SELECT COUNT(*) as count FROM users').get();
    const mailCount = this.db.prepare('SELECT COUNT(*) as count FROM mails').get();
    const totalMailLogs = this.db.prepare('SELECT COUNT(*) as count FROM mail_logs').get();
    return {
      mailboxes: userCount.count,
      totalMails: mailCount.count,
      totalMailsAllTime: totalMailLogs.count  // 削除済みも含む累計
    };
  }
}

...
class MailStore {
  ...
  async getMails(address, password, limit = 100) {
    ...
    const stmt = this.db.prepare(
      'SELECT id, subject, sender, body, html, received_at FROM mails WHERE address = ? ORDER BY received_at DESC LIMIT ?'
    );

...
// HTML/CSS属性値として誤検出される単語（完全マッチで除外）
const HTML_CSS_KEYWORDS = [
  'class', 'style', 'button', 'hidden', 'display', 'center',
  'mobile', 'tablet', 'column', 'content', 'wrapper', 'container',
  'header', 'footer', 'section', 'article', 'sidebar', 'navbar',
  'active', 'disabled', 'loading', 'spinner', 'icon', 'text',
  'left', 'right', 'middle', 'block', 'inline', 'flex', 'grid'
];
...
function isRejected(candidate, fullText) {
  ...
  // ルール9: HTML/CSS属性として使われる単語を排除
  // class="123456" や style="display" などがOTPとして誤認識されるを防ぐ
  const lowerVal = val.toLowerCase();
  if (HTML_CSS_KEYWORDS.includes(lowerVal)) {
    return true;
  }

  // HTML属性パターンの前後をチェック（class="xxx"、style="xxx"）
  const aroundText = fullText.slice(Math.max(0, idx - 20), idx + val.length + 5);
  if (/\b(class|style|id|name)\s*=\s*["']?[^"'>]*$/i.test(beforeText) ||
      /^[^"'<>]*["'>]/.test(fullText.slice(idx + val.length, idx + val.length + 10))) {
    // 属性値として使われている可能性が高い
    if (HTML_CSS_KEYWORDS.some(kw => lowerVal.includes(kw)) || /^[a-z][a-z0-9]*$/i.test(val)) {
      return true;
    }
  }
